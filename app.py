    ...
    if week >= 8:
        ...
        if len(pred_top3) == 3 and len(act_top3) == 3:
            if pred_top3 == act_top3:
                ...
                if len(pred_top3) > 0 and len(act_top3) > 0 and pred_top3[0] == act_top3[0]: score += 3
                if len(pred_top3) > 1 and len(act_top3) > 1 and pred_top3[1] == act_top3[1]: score += 2
                if len(pred_top3) > 2 and len(act_top3) > 2 and pred_top3[2] == act_top3[2]: score += 2
                for idx, baker in enumerate(pred_top3):
                    if baker in act_top3 and (idx >= len(act_top3) or baker != act_top3[idx]):
                        score += 1
        ...
        if len(pred_bottom3) == 3 and len(act_bottom3) == 3:
            if pred_bottom3 == act_bottom3:
                ...
                if len(pred_bottom3) > 0 and len(act_bottom3) > 0 and pred_bottom3[0] == act_bottom3[0]: score += 2
                if len(pred_bottom3) > 1 and len(act_bottom3) > 1 and pred_bottom3[1] == act_bottom3[1]: score += 2
                if len(pred_bottom3) > 2 and len(act_bottom3) > 2 and pred_bottom3[2] == act_bottom3[2]: score += 3
                for idx, baker in enumerate(pred_bottom3):
                    if baker in act_bottom3 and (idx >= len(act_bottom3) or baker != act_bottom3[idx]):
                        score += 1
...
[data-theme="dark"] .lb-table th, .stApp[data-theme="dark"] .lb-table th { background-color: #3E2723 !important; color: #FFFFFF !important; }
[data-theme="dark"] .lb-table td, [data-theme="dark"] .lb-rank, [data-theme="dark"] .lb-name,
.stApp[data-theme="dark"] .lb-table td, .stApp[data-theme="dark"] .lb-rank, .stApp[data-theme="dark"] .lb-name { color: #FFFFFF !important; }
[data-theme="dark"] .lb-pts, .stApp[data-theme="dark"] .lb-pts { color: #FF8A80 !important; }
[data-theme="light"] .lb-table td, [data-theme="light"] .lb-rank, [data-theme="light"] .lb-name,
.stApp[data-theme="light"] .lb-table td, .stApp[data-theme="light"] .lb-rank, .stApp[data-theme="light"] .lb-name { color: #2C1810 !important; }
</style>""", unsafe_allow_html=True)
        ...
        for rank, row in df_lb.iterrows():
            ...
            rows_html_list.append(f'<tr><td class="lb-rank">{rank_badge}</td><td class="lb-name">{m_name}</td><td class="lb-pts">{m_pts} pts</td></tr>')
            
        rows_str = "".join(rows_html_list)
        table_html = f'<table class="lb-table"><thead><tr><th style="width: 100px;">Rank</th><th>League Member</th><th style="text-align: right; width: 140px;">Total Points</th></tr></thead><tbody>{rows_str}</tbody></table>'
...
with tab_admin:
    ...
    if not st.session_state.admin_authenticated:
        ...
        prev_saved_actuals = st.session_state.weekly_results.get(admin_selected_week, {})
        is_week_published = admin_selected_week in st.session_state.weekly_results
        
        st.markdown(f"### Broadcast Results Form for Week {admin_selected_week}")
        if is_week_published:
            st.info(f"🔒 **Week {admin_selected_week} Results Published & Locked.** To modify existing entries, re-enter PIN **6284** below to enable edit mode.")
            unlock_edit = st.text_input(f"Enter PIN 6284 to Unlock Edit Mode for Week {admin_selected_week}:", type="password", key=f"edit_unlock_w{admin_selected_week}")
            form_disabled = (unlock_edit != "6284")
        else:
            form_disabled = False
        ...
        with st.form("admin_actuals_form"):
            ...
            if admin_selected_week == 10:
                actuals["show_champion"] = st.selectbox("Actual Show Champion", admin_baker_options, index=admin_baker_options.index(prev_saved_actuals.get("show_champion")) if prev_saved_actuals.get("show_champion") in admin_baker_options else 0, disabled=form_disabled)
                st.markdown("**Actual Technical Challenge Rankings:**")
                p_tr = prev_saved_actuals.get("tech_rank", ["--Select Baker--"]*3)
                act_t1 = st.selectbox("Actual Technical 1st Place", admin_baker_options, index=admin_baker_options.index(p_tr[0]) if len(p_tr)>0 and p_tr[0] in admin_baker_options else 0, key="act_t1_w10", disabled=form_disabled)
                act_t2 = st.selectbox("Actual Technical 2nd Place", admin_baker_options, index=admin_baker_options.index(p_tr[1]) if len(p_tr)>1 and p_tr[1] in admin_baker_options else 0, key="act_t2_w10", disabled=form_disabled)
                act_t3 = st.selectbox("Actual Technical 3rd Place", admin_baker_options, index=admin_baker_options.index(p_tr[2]) if len(p_tr)>2 and p_tr[2] in admin_baker_options else 0, key="act_t3_w10", disabled=form_disabled)
                actuals["tech_rank"] = [act_t1, act_t2, act_t3]
                
            elif admin_selected_week == 9:
                actuals["star_baker"] = st.selectbox("Actual Star Baker", admin_baker_options, index=admin_baker_options.index(prev_saved_actuals.get("star_baker")) if prev_saved_actuals.get("star_baker") in admin_baker_options else 0, disabled=form_disabled)
                elim_type = st.radio("Elimination Status", ["Single Elimination", "No Elimination (Sickness/Grace Week)", "Double Elimination"], horizontal=True, key="admin_elim_type_w9", disabled=form_disabled)
                if elim_type == "Single Elimination":
                    p_el = prev_saved_actuals.get("eliminated")
                    p_el_str = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else (p_el if isinstance(p_el, str) else "")
                    actuals["eliminated"] = st.selectbox("Actual Eliminated Baker", admin_baker_options, index=admin_baker_options.index(p_el_str) if p_el_str in admin_baker_options else 0, key="act_elim_single_w9", disabled=form_disabled)
                elif elim_type == "No Elimination (Sickness/Grace Week)":
                    actuals["eliminated"] = "None"
                else:
                    p_el = prev_saved_actuals.get("eliminated", ["", ""])
                    p_e1 = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else ""
                    p_e2 = p_el[1] if isinstance(p_el, list) and len(p_el)>1 else ""
                    act_elim_1 = st.selectbox("Actual Eliminated Baker #1", admin_baker_options, index=admin_baker_options.index(p_e1) if p_e1 in admin_baker_options else 0, key="admin_act_elim_1_w9", disabled=form_disabled)
                    act_elim_2 = st.selectbox("Actual Eliminated Baker #2", admin_baker_options, index=admin_baker_options.index(p_e2) if p_e2 in admin_baker_options else 0, key="admin_act_elim_2_w9", disabled=form_disabled)
                    actuals["eliminated"] = [act_elim_1, act_elim_2]
                
                st.markdown("**Actual Technical Challenge Rankings:**")
                p_tr = prev_saved_actuals.get("tech_rank", ["--Select Baker--"]*4)
                act_t1 = st.selectbox("Actual Technical 1st Place", admin_baker_options, index=admin_baker_options.index(p_tr[0]) if len(p_tr)>0 and p_tr[0] in admin_baker_options else 0, key="act_t1_w9", disabled=form_disabled)
                act_t2 = st.selectbox("Actual Technical 2nd Place", admin_baker_options, index=admin_baker_options.index(p_tr[1]) if len(p_tr)>1 and p_tr[1] in admin_baker_options else 0, key="act_t2_w9", disabled=form_disabled)
                act_t3 = st.selectbox("Actual Technical 3rd Place", admin_baker_options, index=admin_baker_options.index(p_tr[2]) if len(p_tr)>2 and p_tr[2] in admin_baker_options else 0, key="act_t3_w9", disabled=form_disabled)
                act_t4 = st.selectbox("Actual Technical 4th Place", admin_baker_options, index=admin_baker_options.index(p_tr[3]) if len(p_tr)>3 and p_tr[3] in admin_baker_options else 0, key="act_t4_w9", disabled=form_disabled)
                ...
                with col1:
                    actuals["star_baker"] = st.selectbox("Actual Star Baker", admin_baker_options, index=admin_baker_options.index(prev_saved_actuals.get("star_baker")) if prev_saved_actuals.get("star_baker") in admin_baker_options else 0, disabled=form_disabled)
                    actuals["in_line_sb"] = st.multiselect("Actual 'In Line' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_line_sb", []) if b in active_bakers], disabled=form_disabled)
                with col2:
                    elim_type = st.radio("Elimination Status", ["Single Elimination", "No Elimination (Sickness/Grace Week)", "Double Elimination"], horizontal=True, key="admin_elim_type_w8", disabled=form_disabled)
                    if elim_type == "Single Elimination":
                        p_el = prev_saved_actuals.get("eliminated")
                        p_el_str = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else (p_el if isinstance(p_el, str) else "")
                        actuals["eliminated"] = st.selectbox("Actual Eliminated Baker", admin_baker_options, index=admin_baker_options.index(p_el_str) if p_el_str in admin_baker_options else 0, key="act_elim_single_w8", disabled=form_disabled)
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_w8", disabled=form_disabled)
                    elif elim_type == "No Elimination (Sickness/Grace Week)":
                        actuals["eliminated"] = "None"
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees (Sickness consolations)", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_w8", disabled=form_disabled)
                    else:
                        p_el = prev_saved_actuals.get("eliminated", ["", ""])
                        p_e1 = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else ""
                        p_e2 = p_el[1] if isinstance(p_el, list) and len(p_el)>1 else ""
                        act_elim_1 = st.selectbox("Actual Eliminated Baker #1", admin_baker_options, index=admin_baker_options.index(p_e1) if p_e1 in admin_baker_options else 0, key="admin_act_elim_1_w8", disabled=form_disabled)
                        act_elim_2 = st.selectbox("Actual Eliminated Baker #2", admin_baker_options, index=admin_baker_options.index(p_e2) if p_e2 in admin_baker_options else 0, key="admin_act_elim_2_w8", disabled=form_disabled)
                        actuals["eliminated"] = [act_elim_1, act_elim_2]
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_w8", disabled=form_disabled)
                    
                st.markdown("**Actual Technical Challenge Rankings:**")
                p_tr = prev_saved_actuals.get("tech_rank", ["--Select Baker--"]*5)
                act_t1 = st.selectbox("Actual Technical 1st Place", admin_baker_options, index=admin_baker_options.index(p_tr[0]) if len(p_tr)>0 and p_tr[0] in admin_baker_options else 0, key="act_t1_w8", disabled=form_disabled)
                act_t2 = st.selectbox("Actual Technical 2nd Place", admin_baker_options, index=admin_baker_options.index(p_tr[1]) if len(p_tr)>1 and p_tr[1] in admin_baker_options else 0, key="act_t2_w8", disabled=form_disabled)
                act_t3 = st.selectbox("Actual Technical 3rd Place", admin_baker_options, index=admin_baker_options.index(p_tr[2]) if len(p_tr)>2 and p_tr[2] in admin_baker_options else 0, key="act_t3_w8", disabled=form_disabled)
                act_t4 = st.selectbox("Actual Technical 4th Place", admin_baker_options, index=admin_baker_options.index(p_tr[3]) if len(p_tr)>3 and p_tr[3] in admin_baker_options else 0, key="act_t4_w8", disabled=form_disabled)
                act_t5 = st.selectbox("Actual Technical 5th Place", admin_baker_options, index=admin_baker_options.index(p_tr[4]) if len(p_tr)>4 and p_tr[4] in admin_baker_options else 0, key="act_t5_w8", disabled=form_disabled)
                ...
                with col1:
                    ...
                    actuals["in_line_sb"] = st.multiselect("Actual 'In Line' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_line_sb", []) if b in active_bakers], disabled=form_disabled)
                with col2:
                    elim_type = st.radio("Elimination Status", ["Single Elimination", "No Elimination (Sickness/Grace Week)", "Double Elimination"], horizontal=True, key="admin_elim_type_std", disabled=form_disabled)
                    if elim_type == "Single Elimination":
                        p_el = prev_saved_actuals.get("eliminated")
                        p_el_str = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else (p_el if isinstance(p_el, str) else "")
                        actuals["eliminated"] = st.selectbox("Actual Eliminated Baker", admin_baker_options, index=admin_baker_options.index(p_el_str) if p_el_str in admin_baker_options else 0, key="act_elim_single_std", disabled=form_disabled)
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_std", disabled=form_disabled)
                    elif elim_type == "No Elimination (Sickness/Grace Week)":
                        actuals["eliminated"] = "None"
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees (Sickness consolations)", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_std", disabled=form_disabled)
                    else:
                        p_el = prev_saved_actuals.get("eliminated", ["", ""])
                        p_e1 = p_el[0] if isinstance(p_el, list) and len(p_el)>0 else ""
                        p_e2 = p_el[1] if isinstance(p_el, list) and len(p_el)>1 else ""
                        act_elim_1 = st.selectbox("Actual Eliminated Baker #1", admin_baker_options, index=admin_baker_options.index(p_e1) if p_e1 in admin_baker_options else 0, key="admin_act_elim_1_std", disabled=form_disabled)
                        act_elim_2 = st.selectbox("Actual Eliminated Baker #2", admin_baker_options, index=admin_baker_options.index(p_e2) if p_e2 in admin_baker_options else 0, key="admin_act_elim_2_std", disabled=form_disabled)
                        actuals["eliminated"] = [act_elim_1, act_elim_2]
                        actuals["in_trouble"] = st.multiselect("Actual 'In Trouble' Nominees", active_bakers, default=[b for b in prev_saved_actuals.get("in_trouble", []) if b in active_bakers], key="admin_trb_std", disabled=form_disabled)
                    
                st.markdown("---")
                st.markdown("**Actual Technical Challenge Rankings:**")
                p_t3 = prev_saved_actuals.get("tech_top_3", ["--Select Baker--"]*3)
                p_b3 = prev_saved_actuals.get("tech_bottom_3", ["--Select Baker--"]*3)
                act_tt1 = st.selectbox("Actual Technical 1st Place", admin_baker_options, index=admin_baker_options.index(p_t3[0]) if len(p_t3)>0 and p_t3[0] in admin_baker_options else 0, key="act_tt1_std", disabled=form_disabled)
                act_tt2 = st.selectbox("Actual Technical 2nd Place", admin_baker_options, index=admin_baker_options.index(p_t3[1]) if len(p_t3)>1 and p_t3[1] in admin_baker_options else 0, key="act_tt2_std", disabled=form_disabled)
                act_tt3 = st.selectbox("Actual Technical 3rd Place", admin_baker_options, index=admin_baker_options.index(p_t3[2]) if len(p_t3)>2 and p_t3[2] in admin_baker_options else 0, key="act_tt3_std", disabled=form_disabled)
                act_tb1 = st.selectbox("Actual Technical 3rd-to-Last Place", admin_baker_options, index=admin_baker_options.index(p_b3[0]) if len(p_b3)>0 and p_b3[0] in admin_baker_options else 0, key="act_tb1_std", disabled=form_disabled)
                act_tb2 = st.selectbox("Actual Technical 2nd-to-Last Place", admin_baker_options, index=admin_baker_options.index(p_b3[1]) if len(p_b3)>1 and p_b3[1] in admin_baker_options else 0, key="act_tb2_std", disabled=form_disabled)
                act_tb3 = st.selectbox("Actual Technical Last Place", admin_baker_options, index=admin_baker_options.index(p_b3[2]) if len(p_b3)>2 and p_b3[2] in admin_baker_options else 0, key="act_tb3_std", disabled=form_disabled)
            ...
            with col_hs1:
                act_handshake_cnt = st.number_input("Number of Handshake Occurrences", min_value=0, value=prev_saved_actuals.get("handshake_count", 0), key=f"admin_hs_cnt_w{admin_selected_week}", disabled=form_disabled)
            with col_hs2:
                act_handshake_stamps = st.text_input("Handshake Circumstances & Video Timestamps", value=prev_saved_actuals.get("handshake_timestamps", ""), placeholder="e.g., Clara @ 14:22 Signature, Tom @ 42:10 Showstopper", key=f"admin_hs_stamps_w{admin_selected_week}", disabled=form_disabled)
            act_handshake_bakers = st.multiselect("Bakers Receiving Hollywood Handshakes", active_bakers, default=[b for b in prev_saved_actuals.get("handshake_bakers", []) if b in active_bakers], key=f"admin_hs_bakers_w{admin_selected_week}", disabled=form_disabled)

            st.markdown("### 😢 Crying Incidents")
            col_cry1, col_cry2 = st.columns(2)
            with col_cry1:
                act_crying_cnt = st.number_input("Number of Crying Occurrences", min_value=0, value=prev_saved_actuals.get("crying_count", 0), key=f"admin_cry_cnt_w{admin_selected_week}", disabled=form_disabled)
            with col_cry2:
                act_crying_stamps = st.text_input("Crying Circumstances & Video Timestamps", value=prev_saved_actuals.get("crying_timestamps", ""), placeholder="e.g., Gabe @ 24:15 Technical, Molly @ 54:02 Elimination", key=f"admin_cry_stamps_w{admin_selected_week}", disabled=form_disabled)

            st.markdown("### 💬 Sexual Innuendos")
            col_inn1, col_inn2 = st.columns(2)
            with col_inn1:
                act_innuendo_cnt = st.number_input("Number of Innuendo Occurrences", min_value=0, value=prev_saved_actuals.get("innuendo_count", 0), key=f"admin_inn_cnt_w{admin_selected_week}", disabled=form_disabled)
            with col_inn2:
                act_innuendo_stamps = st.text_input("Innuendo Circumstances & Video Timestamps", value=prev_saved_actuals.get("innuendo_timestamps", ""), placeholder="e.g., Prue soggy bottom @ 12:10, Paul big nuts comment @ 33:45", key=f"admin_inn_stamps_w{admin_selected_week}", disabled=form_disabled)
            ...
            submit_admin = st.form_submit_button("Publish Official Week Results & Recalculate Scores", disabled=form_disabled)
            if submit_admin and not form_disabled:
                ...
                save_league_data()
                st.success(f"Official results published for Week {admin_selected_week}! Player scores recalculated and active week set to Week {next_week}.")
        ...
        if not st.session_state.disputes:
            ...
            for idx, d in enumerate(st.session_state.disputes):
                ...
                with st.expander(f"Dispute #{idx+1}: {wk} - {cat} ({p_name}) — Status: {curr_status}"):
                    ...
                    current_idx = 0
                    if "Resolved" in curr_status: current_idx = 1
                    elif "Rejected" in curr_status: current_idx = 2
